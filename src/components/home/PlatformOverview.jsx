        ...
        <div className="grid md:grid-cols-2 gap-8 mt-12">
          {products.map((product, i) => (
            <ScrollReveal key={product.name} delay={i * 0.15}>
              <Link
                to={product.link}
                className={`group block bg-white border border-border rounded-2xl p-8 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full border-t-4 ${product.accentBorder}`}
              >
                ...
                <h3 className="font-display text-2xl font-bold text-primary mb-2">{product.name}</h3>
